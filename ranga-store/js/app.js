...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    const image = product.image;  //images problem
...
const addToCart = (price) => {
  ...
  updateTaxAndCharge();
  updateTotal()  // function call hear for total price
...
const updatePrice = (id, value) => {
  ...
  document.getElementById(id).innerText = parseInt(total)//Math.round(total);
};

// set innerText function
const setInnerText = (id, value) => {
  document.getElementById(id).innerText = parseInt(value)  //Math.round(value);
};
